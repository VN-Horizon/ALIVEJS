...
import $ from "jquery";
import { ISceneElement, preserveLayerIndex, SceneElement } from "./SceneElement";
...
export class Background extends SceneElement implements ISceneElement {
  backgroundId: string;
  ...
  updateScrollPosition() {
    if (!this.domElement || ((this.scrollSpeed?.x || 0) == 0 && (this.scrollSpeed?.y || 0) == 0))
      return;
    $(this.layerActive).css(
      "background-position",
      `${this.scrollOffset?.x || 0}px ${this.scrollOffset?.y || 0}px`
    );
  ...
  private _applyTransition(backgroundImageUrl: string | null) {
    ...
    const transitionCss = this.transition
      ? this.transition.replace(/(\d+\.?\d*)s/, `${duration / 1000}s`)
      : `opacity ${duration / 1000}s ease-in-out`;
...
export function toBackground(element: SceneElement | null, options: Partial<BackgroundData> = {}) {
  ...
  if (element.sceneData?.path) background.loadImage(element.sceneData.path).then();
