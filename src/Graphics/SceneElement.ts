...
export function preserveLayerIndex(oldElement: ISceneElement, newElement: ISceneElement) {
  const parent = oldElement.parent;
  let childIndex = -1;
  if (parent) {
    if (oldElement instanceof SceneElement) {
  ...
  if (parent && childIndex >= 0) {
    ...
    if (!(newElement instanceof SceneElement)) return;
