...
import { ACTIVE_NODE_COLOR, CHOICE_NODE_COLOR, CONDITIONAL_LINK_COLOR, DEFAULT_LINK_COLOR, DEFAULT_NODE_COLOR, FUNCTION_NODE_COLOR, SCENE_GRAPH_CHARGE_STRENGTH, SCENE_GRAPH_COLLISION_RADIUS, SCENE_GRAPH_LINK_DISTANCE, SCENE_GRAPH_MAX_ZOOM, SCENE_GRAPH_MIN_ZOOM } from "./SceneGraphPane.constants";
...
export function createSceneGraphRenderer(
    ...
    const renderGraph = (): void => {
        ...
        simulation = d3.forceSimulation<SceneGraphNode>(nodes)
            .alphaDecay(0.001)
            .alpha(2)
            .alphaMin(0)
            .force("link", d3.forceLink<SceneGraphNode, SceneGraphLink>(links)
                .id(d => d.id)
        ...
        const defs = svgElement.selectAll<SVGDefsElement, null>("defs")
            .data([null])
            .join("defs");

        defs.selectAll<SVGMarkerElement, null>("marker#arrowhead")
            .data([null])
            .join("marker")
            .attr("id", "arrowhead")
        ...
        const uniqueColors = [...new Set(links
            .map(link => link.color)
            .filter(color => color !== DEFAULT_LINK_COLOR))];

        uniqueColors.forEach(color => {
            const markerId = `arrowhead-${color.substring(1)}`;
            defs.selectAll<SVGMarkerElement, null>(`marker#${markerId}`)
                .data([null])
                .join("marker")
                .attr("id", markerId)
                .attr("viewBox", "-0 -5 10 10")
        ...
        defs.selectAll<SVGMarkerElement, null>("marker#arrowhead-default")
            .data([null])
            .join("marker")
        ...
        defs.selectAll<SVGMarkerElement, null>("marker#arrowhead-green")
            .data([null])
            .join("marker")
        ...
        const linkSelection = linkGroup
            ...
            .attr("marker-end", d => {
                ...
                return d.color !== DEFAULT_LINK_COLOR ? `url(#arrowhead-${d.color.substring(1)})` : "url(#arrowhead-default)";
            });
...
function createDrag(simulation: d3.Simulation<SceneGraphNode, SceneGraphLink>) {
    return d3.drag<SVGGElement, SceneGraphNode>()
