...
function renderGraph(svgElement, linkGroup, nodeGroup, width, height) {
    ...
    // Update nodes
    const nodeSelection = nodeGroup.selectAll("g").data(nodes).join("g").call(createDrag(simulation));
...
function updateCurrentEventHighlight() {
    ...
    if (currentNode && currentNode.x && currentNode.y) {
        ...
        svgElement.transition().duration(750).call(svgElement.zoom.transform, transform);
    }
}
...
function getEventsGraphData() {
    ...
    const nodesPointingToEnd = links.filter(link => link.target === 7090644).map(link => link.source);
