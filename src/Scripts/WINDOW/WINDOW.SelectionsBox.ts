...
export async function initSelectionsBox(dialogWindow: DialogWindowScene | null) {
  ...
  const openSelectionsBox = (numChoices: number) => {
    ...
    frameAnim.onComplete = () => {
      ...
      selectionsText?.show();
      Array.from({ length: numChoices }, (_, i) => selectionButtons[i]?.show());
    };
    frameAnim.play();
  };
  ...
  document.addEventListener("ShowDecisionInternal", (e: Event) => {
    const choices = (e as CustomEvent).detail.stringParams;
    openSelectionsBox(choices.length);
    // Translate
    const translatedContents = choices.map((key: string) => $translate(key));
    selectionsText?.setText(translatedContents.join("\n"));
  });
}
