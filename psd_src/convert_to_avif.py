...
def convert_png_to_avif(png_path):
    result = {
        'success': False,
        'error': None,
        'original_size': 0,
    ...
    try:
        ...
        with Image(filename=str(png_path)) as img:
            img.format = 'avif'
            img.options['avif:speed'] = str(AVIF_METHOD)
            img.compression_quality = AVIF_QUALITY
            img.save(filename=str(avif_path))
        
        result['new_size'] = get_file_size(avif_path)
...
def main():
    ...
    print(f"Quality: {AVIF_QUALITY}, Method: {AVIF_METHOD}")
    print()
    ...
    for idx, png_file in enumerate(png_files, 1):
        ...
        result = convert_png_to_avif(png_file)
